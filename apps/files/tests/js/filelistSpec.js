...
describe('FileList tests', function() {
	...
	beforeEach(function() {
		...
		routerGenerateStub = sinon.stub(OC.Router, 'generate', function(module, spec) {
			if (spec && spec.file) {
				...
				return '/index.php/apps/files/' + module;
			}
		});
	...
	describe('Adding files', function() {
		...
		it('generates file element with correct attributes when calling add() with file data', function() {
			...
			var $tr = FileList.add(fileData, {linkUrl: 'test/download/url'});
			...
			expect($tr.find('a.name').attr('href')).toEqual('test/download/url');
	...
	describe('List rendering', function() {
		...
		it('do not add file if in different directory', function() {
			$('#dir').val('/current dir');
			var fileData = {
				type: 'file',
				name: 'testFile.txt',
			...
			var $tr = FileList.add(fileData);
			expect(FileList.findFileEl('testFile.txt').length).toEqual(0);
		});
	...
	describe('file previews', function() {
		...
		it('renders default icon for file when none provided and no preview is available', function() {
			...
			expect(getImageUrl($td)).toEqual(OC.webroot + '/core/img/filetypes/file.png');
		...
		it('renders default icon for dir when none provided and no preview is available', function() {
			...
			var $tr = FileList.add(fileData);
			var $td = $tr.find('td.filename');
			expect(getImageUrl($td)).toEqual(OC.webroot + '/core/img/filetypes/folder.png');
			expect(previewLoadStub.notCalled).toEqual(true);
		});
		it('renders provided icon for file when provided', function() {
			var fileData = {
				type: 'file',
				name: 'test dir',
				icon: OC.webroot + '/core/img/filetypes/application-pdf.png'
			};
			var $tr = FileList.add(fileData);
			var $td = $tr.find('td.filename');
			expect(getImageUrl($td)).toEqual(OC.webroot + '/core/img/filetypes/application-pdf.png');
			expect(previewLoadStub.notCalled).toEqual(true);
		});
